...
namespace AHD.ID3
{
    ...
    public class ID3v1
    {
        ...
        public Result Save(string fileName)
        {
            ...
            j = 0;
            string backupName = tempFolder + Path.GetFileNameWithoutExtension(fileName) + "_backup" + j;
            while (File.Exists(backupName))
            {
                j++;
                backupName = tempFolder + Path.GetFileNameWithoutExtension(fileName) + "_backup" + j;
            }
            ...
            catch (Exception ex)
            {
                ...
                DebugLogger.WriteLine("ORIGNAL BACKUP FILE IS SAVED AT : " + backupName, DebugCode.Error);
                // close streams
                fileStream.Close();
                fileStream.Dispose();
                tempStream.Close();
