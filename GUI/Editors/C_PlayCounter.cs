...
{
    public partial class C_PlayCounter : EditorControl
    {
        public C_PlayCounter()
        {
            InitializeComponent();
            numberControl1.MaximumNumber = int.MaxValue;
            DisableControls();
        }
        private PlayCounterFrame frame;
        private void DisableControls()
        {
            numberControl1.Enabled = toolStrip1.Enabled = false;
            Label1.Text = "";
        }
        ...
        {
            numberControl1.Enabled = toolStrip1.Enabled = true;
            Label1.Text = "";
        }
        ...
        {
            numberControl1.PropertyValue = 0;
        }
        /// <summary>
        ...
        public void LoadFrames(ID3v2 v2)
        {
            ...
            if (v2.TagVersion.Major == 2)
                frame = (PlayCounterFrame)v2.GetFrameLoaded("CNT");
            else
                frame = (PlayCounterFrame)v2.GetFrameLoaded("PCNT");

            if (frame != null)
                numberControl1.PropertyValue = frame.Counter;
            else
                numberControl1.PropertyValue = 0;
        }
        ...
        {
            // remove all frames
            v2.RemoveFrameAll("PCNT");
            v2.RemoveFrameAll("CNT");
            // add new
            if (numberControl1.PropertyValue > 0)
            {
                if (v2.TagVersion.Major == 2)
                    frame = new PlayCounterFrame("CNT", "Play counter", null, 0);
                else
                    frame = new PlayCounterFrame("PCNT", "Play counter", null, 0);
                frame.Counter = (int)numberControl1.PropertyValue;
                v2.Frames.Add(frame);
            }
        }
